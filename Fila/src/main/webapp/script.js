...
document.querySelector(".inicio").textContent = "Inicio da fila: " + posicaoDesenfileirar;
document.querySelector(".fim").textContent = "Fim da fila: " + posicaoEnfileirar;
...
function enfileirarNumero() {
  ...
  if (!isNaN(numero)) { // Verifica se o número é válido
    if (posicaoEnfileirar !== fimFila) { // Verifica se o último setor está ocupado
      ...
      exibirFila(); // Exibe a fila no console
      // Verificar estado da fila
      var estadoFila = verificarEstadoFila();
      // Enviar estadoFila para o código JSP usando AJAX
      var xhr = new XMLHttpRequest();
      xhr.open('POST', 'index.jsp', true);
      xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
      xhr.send('estadoFila=' + estadoFila);
    } 
  }
...
function verificarEstadoFila() {
  if (posicaoEnfileirar === posicaoDesenfileirar && fila.length > 0) {
	console.log('Estado da fila:', 'vazia');
    return 'vazia';
  }
}

function exibirMensagemSaida(mensagem) {
  var caixaSaida = document.querySelector('.caixa-saida');
  caixaSaida.textContent = mensagem;
}
