...
var filaCheia = false; // Verificar se a fila está cheia
...
window.addEventListener('load', function() {
  ...
  if (tamanho < 2 || tamanho > 10) {
    registros.push({ tipo: 'erro', mensagem: 'Erro: O tamanho deve estar entre 2 e 10' });
  ...
  btnEnfileirar.addEventListener('click', function() {
	  var valor = parseInt(document.querySelector('.input-valor').value);
    	if (valor <= 0 || valor > 99) {
    registros.push({ tipo: 'erro', mensagem: 'Erro: O valor deve estar entre 1 e 99' });
    atualizarCaixaSaida();
    return;
  }
    enfileirarNumero();
